...
def to_grid(rgbs):
    ...
    font = ImageFont.truetype("Roboto/Roboto-Regular.ttf", font_size)
...
def get_colors(image, maxcolors=4, max_tolerance=6):
    ...
    for color in colors:
        ...
        for col in colors:
            ...
            diff = get_difference(rgb1, rgb2)
            if diff < max_tolerance:
                similar.append(col)
                scanned.add(col)

        similar_colors.append(similar)
    ...
    for similar in similar_colors:
        ...
        for i in range (1, len(similar)):
            color = similar[i]
            new_color = (new_color[0] + color[0], new_color[1])
